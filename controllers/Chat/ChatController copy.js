...
const sellerCustomerMessage = require("../../models/chat/sellerCustomerMessage"); // path ঠিক আছে তো?
...
class chatController {
    ...
    seller_admin_message_insert = async (req, res) => {
        ...
        const senderId = req.id; // authMiddleware থেকে সেট করা

        if (!senderId) {
            return responseReturn(res, 401, { error: 'Please Login first' });
        }
        if (!receverId || !message) {
            return responseReturn(res, 400, { error: 'receverId and message are required' });
        }
