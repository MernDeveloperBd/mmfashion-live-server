...
dbConnect()
 ...
 app.use(cors({
    origin:process.env.mode === 'pro' ? [process.env.CLIENT_URL, process.env.CLIENT_ADMIN_PRO_URL]:['http://localhost:5173','http://localhost:5174'],
    credentials: true,
...
/*     app.use(cors({
    origin:[process.env.CLIENT_URL, process.env.CLIENT_ADMIN_PRO_URL],
    credentials: true,
    methods: ['GET', 'POST', 'PUT', 'DELETE', 'PATCH', 'OPTIONS'],
    allowedHeaders: ['Content-Type', 'Authorization']
}));
